...
for name in data_list:
    ...
    hp_dt = {"dt__min_samples_split" : list(range(2, 51)), "dt__min_samples_leaf" : list(range(2, 51)), "dt__max_depth" : list(range(2, 31))}
    ...
    for m, h in zip(models, hyperparameters):
        ...
        original['dataset'].append(name)


        # pipe vt
        pipe_vt = Pipeline(steps=[('vt', vt), (str(models2[i]), m)])
        search_vt = RandomizedSearchCV(pipe_vt, {**hp_vt, **h}, n_jobs=-1, cv=stkf, n_iter= nruns, scoring='balanced_accuracy')
        search_vt.fit(X, y)
        ...
        variance['percent'].append((X.shape[1] - sum(search_vt.best_estimator_.named_steps['vt'].get_support()))/(X.shape[1] + sum(search_vt.best_estimator_.named_steps['vt'].get_support())))
        variance['dataset'].append(name)



        # pipe chi
        pipe_chi = Pipeline(steps=[('chi', chi), (str(models2[i]), m)])
        search_chi = RandomizedSearchCV(pipe_chi, {**hp_chi, **h}, n_jobs=-1, cv=stkf, n_iter= nruns, scoring='balanced_accuracy')
        search_chi.fit(X, y)
        ...
        chisquared['percent'].append((X.shape[1] - sum(search_chi.best_estimator_.named_steps['chi'].get_support()))/(X.shape[1] + sum(search_chi.best_estimator_.named_steps['chi'].get_support())))
        chisquared['dataset'].append(name)


        # pipe f_classif
        pipe_f_cla = Pipeline(steps=[('f_cla', f_cla), (str(models2[i]), m)])
        search_f_cla = RandomizedSearchCV(pipe_f_cla, {**hp_f_cla, **h}, n_jobs=-1, cv=stkf, n_iter= nruns, scoring='balanced_accuracy')
        ...
        anova['percent'].append((X.shape[1] - sum(search_f_cla.best_estimator_.named_steps['f_cla'].get_support()))/(X.shape[1] + sum(search_f_cla.best_estimator_.named_steps['f_cla'].get_support())))
        anova['dataset'].append(name)

        # pipe pca
        pipe_pca = Pipeline(steps=[('pca', pca), (str(models2[i]), m)])
        search_pca = RandomizedSearchCV(pipe_pca, {**hp_pca, **h}, n_jobs=-1, cv=stkf, n_iter= nruns, scoring='balanced_accuracy')
        search_pca.fit(X, y)
        ...
        principal['percent'].append((X.shape[1] - search_pca.best_estimator_.named_steps['pca'].n_components_)/(X.shape[1] + search_pca.best_estimator_.named_steps['pca'].n_components_))
        principal['dataset'].append(name)

        i+=1

pd.DataFrame(original).to_csv("original.csv", index=False)
pd.DataFrame(variance).to_csv("vt.csv", index=False)
pd.DataFrame(chisquared).to_csv("chi.csv", index=False)
pd.DataFrame(anova).to_csv("f_cla.csv", index=False)
pd.DataFrame(principal).to_csv("pca.csv", index=False)
